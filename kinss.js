...
function updateHomeImage() {
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    reader.onload = function(e) {
      // Temporarily update the image (will reset on refresh)
      document.getElementById('home-img-1').src = e.target.result;
      alert('Image updated (temporarily - will reset on refresh)');
    };
    reader.readAsDataURL(fileInput.files[0]);
  }
}

function updateTouristImage() {
  const fileInput = document.getElementById('tourist-image-upload');
  if (fileInput.files && fileInput.files[0]) {
    const reader = new FileReader();
    reader.onload = function(e) {
      // Temporarily update the image (will reset on refresh)
      document.getElementById('tourist-img-1').src = e.target.result;
      alert('Image updated (temporarily - will reset on refresh)');
    };
    reader.readAsDataURL(fileInput.files[0]);
  }
